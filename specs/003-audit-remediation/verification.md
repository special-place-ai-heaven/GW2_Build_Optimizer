...
- Green: `cargo test --workspace --no-fail-fast` at 1f85f27, all suites pass. Two tests were noted as flakes: `gw2-api client::tests::fetch_bytes_rejects_a_body_over_the_icon_cap` and `gw2-optimizer scraper::tests::scrape_guildjen_aborts_at_inner_loop_when_cancelled` (500 ms budget, mostly reqwest client construction; predates this campaign at d88cb32).
- Correction 2026-09-06 (W268): the icon-cap test is **not** a parallelism flake. It failed under `--test-threads=1`, passed in the parallel run, and passed 6/6 in isolation — order / accumulated state between `gw2api` tests. Open as W268. The GuildJen abort test is unchanged (timing budget).
