...
- `CancellationToken` system added (6abcb61): replaced static SHUTDOWN flag with `Arc<AtomicBool>` token stored in `AddonState`; all 8 background thread spawn sites clone and check token; `clear()` cancels before drop — prevents dangling threads on addon unload
<!-- END AUTO-MANAGED -->
