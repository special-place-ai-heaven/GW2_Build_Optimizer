...
crates/addon/       — cdylib: Nexus entry point, ImGui UI, keybinds (nexus-rs)
  ...
  src/ui/main_view.rs — full main view: left menu (character picker, Build Template dropdown, Equipment Template dropdown, chat code display with Copy button, game mode radio, tab nav), render_new_build_tab() (archetype selector + comparison + chat bar), render_improve_tab(), render_saveload_tab() (lists/loads/deletes saved builds), render_settings_tab(); render_save_build_ui() shown below comparison when suggestions exist; start_optimization() → engine::optimize() → enrich_with_gemini() pipeline; enrich_with_gemini() uses generate_with_tools() with ToolContext giving Gemini access to all 12 game data tools; send_chat_message() also uses generate_with_tools() with GameDb clone for tool-enabled chat; apply_gemini_response() non-destructive merge onto BuildSuggestion; summarize_resolved_build() produces text block for LLM context; infer_archetype_from_build() subtracts base stats, covers all 7 archetypes; perf_to_combat_metrics() and compute_3tier_combat() DRY helpers for CombatPerformance→CombatMetrics conversion; calculate_current_stats() returns CombatBundle (StatBlock + 3-tier combat metrics) — extracts damage modifiers from equipped traits/items and computes all buff tiers for the current build
...
crates/optimizer/   — engine.rs (pipeline orchestration; BuildCandidate carries combat: CombatPerformance + modifiers: DamageModifiers + equipped_traits: Vec<u32>; scoring via score_combat() using CombatPerformance, not raw stats; select_best_major_traits() picks 1 best per column), gamedb.rs (GameDb: pre-indexed HashMap lookups for all game data, derived indexes; includes pvp_amulets: HashMap<u32, PvpAmulet>), combat.rs (DamageModifiers: multiplicative strike/condi/crit/healing modifiers + additive duration modifiers; BuffProfile: might_stacks/fury/protection/resolution/vulnerability_stacks/label; CombatPerformance: effective_power, crit_chance, strike/condition/total DPS indexes, condition_ticks: ConditionTicks, healing_power_index, boon/condi duration, effective_health, damage_reduction_pct; calculate_combat_performance() applies buff stats + modifiers using GW2 published formulas — EHP = health × armor / REFERENCE_ARMOR / (1 - protection_DR), armor is LINEAR divisor not DR curve; calculate_condition_ticks() applies 5 tick formulas with per-condition modifiers; extract_damage_modifiers() parses Fact::AttributeAdjust/Buff from traits+items; default_buff_profiles() returns [Solo (no buffs), Party (Might x15 + Fury), Full Squad (Might x25 + Fury + Vuln x25)]; REFERENCE_ARMOR=2597.0, REFERENCE_WEAPON_STRENGTH=1100.0), scoring.rs (Archetype enum with score_combat + score_stats + relevant_prefixes), gemini.rs (GeminiClient: gemini-2.5-flash; generate/generate_cached/generate_with_tools; FunctionCall/FunctionResponse/Tool/FunctionDeclaration types for Gemini function calling; multi-turn loop with max_turns cap; RateTracker: 10 RPM, 240/250 RPD), gemini_tools.rs (ToolContext + 12 tools exposed to Gemini: get_profession_info, get_spec_traits, get_trait_details, get_skill_info, list_runes, list_sigils, list_relics, calculate_stats, simulate_combat, score_build, get_current_build, get_optimizer_results; tool_declarations() + execute_tool() dispatcher), prompts.rs (tool-aware prompts: new_build_prompt_with_tools, improve_build_prompt_with_tools, chat_refinement_prompt_with_tools + legacy fallbacks; parse_gemini_build returns typed GeminiBuildResponse), search.rs (6 mix strategies: full set, trinket split, rings only, accessories only, weapons only, armor-only primary), stats.rs (StatBlock with AddAssign impl)
```
