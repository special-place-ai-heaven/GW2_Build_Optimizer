...
**GW2 Build Optimizer v1.4.16** — In-game Guild Wars 2 addon (Nexus plugin) that optimizes character builds across all game modes (PvE, PvP, WvW). Uses the GW2 API for game/character data and a pluggable LLM backend (Gemini, OpenAI, Anthropic, or OpenRouter) for build reasoning. Feature complete (S01-S15). Overlay chrome is localized (Settings → Language). Skill/trait/item names follow official API `lang=` for Deutsch, Español, Français, and 简体中文. Land Spear is a terrestrial two-hander (API Aquatic flag is the underwater palette, not a land reject). Choya plates kits with stats and keeps the composer editable while a request is in flight.
